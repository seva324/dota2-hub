...
export function MatchDetailModal({ matchId, seriesMaps = [], open, onOpenChange, onTeamClick, onPlayerClick, fullPage = false }: MatchDetailModalProps) {
  ...
  useEffect(() => {
    if (activeMatchId && open) {
      ...
      fetchMatchDetailModel(matchIdNum, seriesMaps)
        .then((data) => {
          if (cancelled) return;
          setMatch(data);
        })
  ...
  const detailModel = deriveMatchDetailModel(match, seriesMaps);
  const radiantPlayers = detailModel.radiantPlayers;
  const direPlayers = detailModel.direPlayers;
  const radiantTeamName = detailModel.radiantTeamName;
  const direTeamName = detailModel.direTeamName;
  const radiantTeamRef = detailModel.radiantTeamRef;
  const direTeamRef = detailModel.direTeamRef;
  const radiantSeriesWins = detailModel.radiantSeriesWins;
  const direSeriesWins = detailModel.direSeriesWins;
