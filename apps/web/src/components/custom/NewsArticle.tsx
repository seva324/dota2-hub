...
export function formatNewsDate(timestamp: number) {
  if (!timestamp) return '';
...
export function RichContent({ raw }: { raw: string }) {
...
export function pickRelated(item: NewsItem, pool: NewsItem[]): NewsItem[] {
...
/** 单条新闻完整阅读视图（全屏页正文区） */
export function NewsArticleView({ item, pool, onSelectRelated }: NewsArticleViewProps) {
  ...
  const relatedItems = useMemo(() => pickRelated(item, pool), [item, pool]);
  ...
  return (
    <div className="min-h-0 flex-1">
      <div className="px-6 pb-16 pt-8 sm:px-10">
        ...
        <h1 className="mt-4 max-w-[24em] text-pretty text-[26px] font-extrabold leading-[1.3] tracking-[-0.015em] text-white sm:text-[34px]">
          {item.title}
        </h1>
        {item.summary && (
          <p className="mt-3.5 max-w-[38em] text-[15.5px] leading-[1.75]" style={{ color: design.text2 }}>
            {item.summary}
          </p>
        )}
        ...
        {relatedItems.length > 0 && (
          <section className="mt-12" aria-label="相关阅读">
            <h2 className="mb-5 text-[19px] font-extrabold tracking-[-0.01em] text-white">相关阅读</h2>
