...
session="d2hub-live-hero-persist"
...
base_url="${base_url%/}"
# 低频持久化：hero_live_scores（监控快照）由本脚本每 5min 维护一次，
# 高频 live-hero/live-detail 请求路径已去 Neon，无需在本脚本里高频唤醒 compute。
endpoint="persist-live-hero|${base_url}/api/cron?action=persist-live-hero&minIntervalMin=${min_interval_min}"
