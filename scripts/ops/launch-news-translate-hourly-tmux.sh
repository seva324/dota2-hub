...
for arg in "$@"; do
  case "$arg" in
    ...
    --env-file=*) if [[ "$env_files_set" == "0" ]]; then env_files=(); env_files_set="1"; fi; env_files+=("${arg#*=}") ;;
...
if (( ${#env_files[@]} == 0 )); then
  env_files=(".env.vercel" ".env.translate")
fi
...
for env_file in "${env_files[@]}"; do
  ...
  resolved_env_files+=("$resolved_env_file")
done
...
echo "env_files=${resolved_env_files[*]:-<process-env>}"
