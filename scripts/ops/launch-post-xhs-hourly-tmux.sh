...
runner_cmd="cd $(printf '%q' "$ROOT_DIR") && export XHS_CODEX_MODEL=gpt-5.4-mini && sleep 300 && while true; do printf '\n===== %s XHS post start =====\n' \"\$(date '+%Y-%m-%d %H:%M:%S %Z')\" >> $(printf '%q' "$log_file"); node --env-file=.env.vercel scripts/post-news-to-xhs.mjs --limit $(printf '%q' "$post_limit") --require-complete-translation >> $(printf '%q' "$log_file") 2>&1; status=\$?; printf '===== %s XHS post exit:%s =====\n' \"\$(date '+%Y-%m-%d %H:%M:%S %Z')\" \"\$status\" >> $(printf '%q' "$log_file"); sleep $(printf '%q' "$(( interval_min * 60 ))"); done"
