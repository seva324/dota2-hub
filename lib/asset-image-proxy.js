...
const STEAM_HOST_ALTERNATES = {
  'cdn.steamstatic.com': 'cdn.cloudflare.steamstatic.com',
  'cdn.cloudflare.steamstatic.com': 'cdn.steamstatic.com',
};

const DLTV_HOST_ALTERNATES = {
  'dltv.org': 's3.dltv.org',
  's3.dltv.org': 'dltv.org',
  'www.dltv.org': 's3.dltv.org',
};

export function getDltvHostAlternate(rawUrl, baseUrl = DEFAULT_BASE_URL) {
  const normalized = normalizeAssetProxySourceUrl(rawUrl, baseUrl);
  if (!normalized) return null;
  try {
    const url = new URL(normalized);
    const alternate = DLTV_HOST_ALTERNATES[url.hostname.toLowerCase()];
...
export function getSteamHostAlternate(rawUrl, baseUrl = DEFAULT_BASE_URL) {
  const normalized = normalizeAssetProxySourceUrl(rawUrl, baseUrl);
  if (!normalized) return null;
  try {
    const url = new URL(normalized);
    const alternate = STEAM_HOST_ALTERNATES[url.hostname.toLowerCase()];
    if (!alternate) return null;
    url.hostname = alternate;
    return url.toString();
  } catch {
...
export function isSupportedAssetImageUrl(rawUrl, baseUrl = DEFAULT_BASE_URL) {
  ...
  try {
    ...
    if (DLTV_HOSTS.has(host)) {
      ...
      return /^\/(?:uploads\/(?:players|teams|events|heroes|items)\/|images\/teams\/)/i.test(url.pathname);
    }
...
export function getAssetImageFetchCandidates(rawUrl, baseUrl = DEFAULT_BASE_URL) {
  ...
  try {
    ...
    } else if (host === 's3.dltv.org' && /^\/uploads\/(?:players|teams|events|heroes|items)\//i.test(url.pathname)) {
      appendUnique(candidates, `https://dltv.org${url.pathname}${url.search}`);
      appendUnique(candidates, normalized);
    } else if (host === 'dltv.org' && /^\/uploads\/(?:players|teams|events|heroes|items)\//i.test(url.pathname)) {
      appendUnique(candidates, normalized);
