...
export function parseDltvEventPage(raw, sourceUrl) {
  ...
  const location = markdown
    ...
    : (extractLabelValue(text, 'COUNTRY') || extractLabelValue(text, 'LOCATION'));
  ...
  const prizePool = normalizePrizePool(
    (markdown ? extractMarkdownLabelValue(raw, 'Prize pool') : extractLabelValue(text, 'PRIZE POOL'))
  ...
  return {
    ...
    tier: parseTier(
      markdown
        ...
        : (extractLabelValue(text, 'EVENT TIER') || extractLabelValue(text, 'TIER'))
    ),
