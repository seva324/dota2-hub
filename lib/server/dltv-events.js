...
const CANONICAL_REGION_TOKENS = new Set(Array.from(REGION_ALIAS_MAP.values()));
    ...
    try {
      const res = await fetchImpl(attempt.url, {
        headers: {
          ...
          ...(attempt.type === 'jina' ? { 'X-No-Cache': 'true' } : {}),
        },
...
export function scoreTournamentNameMatch(eventName, candidateName) {
  ...
  const eventRegionTokens = eventTokens.filter((token) => CANONICAL_REGION_TOKENS.has(token));
  const candidateRegionTokens = candidateTokens.filter((token) => CANONICAL_REGION_TOKENS.has(token));
  ...
  const regionOverlap = countOverlap(eventRegionTokens, candidateRegionTokens);
  ...
  const hasEditionMismatch = eventNumberTokens.length > 0
    ...
    && numberOverlap === 0;
  const hasRegionMismatch = eventRegionTokens.length > 0
    && candidateRegionTokens.length > 0
    && regionOverlap === 0;
...
function parseEventsCatalogMarkdown(raw) {
  ...
  for (const match of String(raw || '').matchAll(/\[([\s\S]*?)\]\((https?:\/\/dltv\.org\/events\/[^)\s]+)\)/g)) {
    ...
    entries.push({
      sourceUrl,
      title: parsed.title,
      status: deriveTournamentStatus(null, null),
      tier: parsed.tier,
...
function parseEventsCatalogCardHtml(block, sourceUrl) {
  ...
  return {
    ...
    status: deriveTournamentStatus(
      parseDateTimeSource(dateSources[0]),
      parseDateTimeSource(dateSources[1], true),
    ),
...
export function parseDltvEventPage(raw, sourceUrl) {
  ...
  return {
    ...
    status: deriveTournamentStatus(dateRange.startTime || null, dateRange.endTime || null),
    tier: parseTier(
      markdown
