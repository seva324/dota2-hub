...
function matchRoleKey(key, roled) {
  ...
  for (const [name, roleKey] of roled) {
    if (name.length >= 3 && (norm.includes(name) || name.includes(norm))) return roleKey;
  }
  return undefined;
}
  ...
  if (!task) {
    task = (async () => {
      ...
      for (const p of players) {
        ...
        if (p.nick) byName.set(normalizeNick(p.nick), roleKey);
        if (p.realName) byName.set(normalizeNick(p.realName), roleKey);
      }
      if (byName.size === 0) {
