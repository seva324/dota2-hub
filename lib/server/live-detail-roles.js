...
import {
  ...
  readTeamSquadCache,
  writeTeamSquadCache,
} from './dltv-squad-cache.js';

const TEAM_PAGE_URL = 'https://dltv.org/teams/';
const CACHE_TTL_MS = 30 * 60 * 1000;
// Neon 里 squad 数据的变化极低（转会/换人），6h 新鲜窗口足够。
...
/** 无 DLTV roster 数据队伍的已知选手位置（归一化 key → 1~5 号位）。
 *  HULIGANI（L1GA Team，DLTV 页面无 Active squad）——依据 TI 2026 小组赛
 *  实际出装英雄位置（Sven/Drow=1、SF/Invoker=2、Centaur=3、ES/DW=4、Undying/ET=5）。 */
const HAWK_PLAYER_POSITIONS = new Map([
  ['ssnovv1', 1],
  ['mirage', 2],
  ['corrupted', 3],
  ['sayuw', 4],
 ...
 *  squad 为空（DLTV 无该队 roster）时用 HAWK_PLAYER_POSITIONS 直给。 */
function matchRoleKey(key, roled) {
  const norm = normalizeNick(key);
  if (!norm) return undefined;
  if (roled.size === 0) {
    const direct = HAWK_PLAYER_POSITIONS.get(norm);
 ...
 *  Neon 优先（本地 cron 预热，EdgeOne 出口抓 dltv 不稳）；miss 再抓取并回写 Neon。 */
export async function getSquadRoleMap(teamName) {
  ...
  if (db) {
    try {
      ...
      const dbCached = await readTeamSquadCache(db, slug);
      const refreshedAt = Number(dbCached?.refreshedAt || 0);
      if (Array.isArray(dbCached?.payload)
        && refreshedAt > 0
        && Date.now() - refreshedAt < DB_CACHE_TTL_MS) {
        const byName = new Map(dbCached.payload);
        if (byName.size > 0) {
      ...
      console.warn('[live-detail-roles] neon squad read failed:', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  if (!task) {
    task = (async () => {
      ...
      if (byName.size === 0) {
        ...
        // 抓取成功回写 Neon：本地 cron / 任意一次成功请求都能为 EdgeOne 共享缓存。
        await writeTeamSquadCache(db, slug, [...byName.entries()]);
      }
      return byName;
    })();
