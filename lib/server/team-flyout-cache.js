...
let ensureTeamFlyoutCacheTablePromise = null;
...
export async function ensureTeamFlyoutCacheTable(db) {
  if (ensureTeamFlyoutCacheTablePromise) {
    return ensureTeamFlyoutCacheTablePromise;
  }

  ensureTeamFlyoutCacheTablePromise = (async () => {
    await db.query(`
    ...
    `);
    await db.query(`
      CREATE INDEX IF NOT EXISTS idx_team_flyout_cache_updated_at
      ON team_flyout_cache(updated_at DESC)
    `);
  })().catch((error) => {
    ensureTeamFlyoutCacheTablePromise = null;
    throw error;
  });
...
export async function warmTeamFlyoutCache(db, options = {}) {
  ...
  await mapWithConcurrency(teams, concurrency, async (row) => {
    try {
