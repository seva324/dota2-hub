...
export function parseDltvUpcomingMatchesMarkdown(markdown, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    upcoming.push({
      ...
      matchUrl: normalizeUrl(matchLink[1]),
      radiantName,
      direName,
      tournament,
...
export function parseDltvUpcomingMatchesPage(html, options = {}) {
  ...
  const upcoming = [];
  // DLTV 的 match__head-event（赛事名/格式）只在每组赛事的第一场前出现一次，
  // 后续连续场次没有自己的赛事名。之前固定回溯 4000 字符，连续 3 场以上时
  // 后面几场捞不到赛事名被丢弃（表现为 upcoming 只显示前 2 场）。
  // 改为按文档顺序 carry-forward 上一个赛事组的上下文。
  ...
  for (const block of collectUpcomingBlocks(source)) {
    ...
    const headTournament = extractHeadTournament(block.html);
    const blockFormats = extractFormatTexts(block.html);
    const blockEventUrl = extractEventUrl(block.html);

    if (headTournament) {
      carriedContext = {
        tournament: headTournament,
        eventUrl: blockEventUrl || (carriedContext?.eventUrl || null),
        formatTexts: blockFormats.length > 0 ? blockFormats : (carriedContext?.formatTexts || []),
      };
    }

    if (!carriedContext) {
      // 首个 block 无自带赛事名时，用历史回溯窗口兜底一次
      const contextFallback = extractContextFallback(source, block.startIndex);
      carriedContext = {
        tournament: contextFallback.tournament,
        eventUrl: contextFallback.eventUrl,
        formatTexts: contextFallback.formatTexts,
      };
    }

    if (!carriedContext.tournament) continue;

    const effectiveFormatTexts = blockFormats.length > 0 ? blockFormats : carriedContext.formatTexts;
    ...
    upcoming.push({
      ...
      radiantName,
      direName,
      tournament: carriedContext.tournament,
      eventUrl: blockEventUrl || carriedContext.eventUrl,
      stage,
      bestOf,
      timestamp,
    });
