...
// 任何 match 卡片（live/upcoming/finished）的起始，用于限定最后一个块的边界
const ANY_MATCH_BLOCK_RE = /<div\b[^>]*class=(["'])[^"']*\bmatch\b[^"']*\b(live|upcoming|finished)\b[^"']*\1[^>]*>/gi;

function collectBlocks(html, blockClassRegex) {
  const source = String(html || '');
  const matches = [...source.matchAll(blockClassRegex)];
  const anyStarts = [...source.matchAll(ANY_MATCH_BLOCK_RE)].map((m) => Number(m.index) || 0);

  return matches.map((match, index) => {
    const startIndex = Number(match.index) || 0;
    let endIndex = index + 1 < matches.length ? matches[index + 1].index : source.length;
    if (index === matches.length - 1) {
      // 最后一个块：截到下一个任意 match 卡片，避免吞入页尾的 JS/i18n 数据
      const nextAny = anyStarts.find((pos) => pos > startIndex);
      if (nextAny) endIndex = nextAny;
    }
    return {
      startIndex,
      openingTag: match[0],
      html: source.slice(startIndex, endIndex),
    };
  });
}
...
function extractTeams(html) {
  // 按 match__body-details__team 块归属提取；用"下一个 team 块或 watch 区"作边界，
  // 不依赖精确的 </div> 闭合数（HTML 结构可能因抓取源不同而变化）
  const source = String(html || '');
  const startRe = /<div\b[^>]*class=(["'])[^"']*\bmatch__body-details__team\b[^"']*\1[^>]*>/gi;
  const starts = [...source.matchAll(startRe)];
  const teams = [];

  for (let index = 0; index < starts.length && teams.length < 2; index += 1) {
    const blockStart = starts[index].index;
    const nextStart = index + 1 < starts.length ? starts[index + 1].index : -1;
    const blockEnd = nextStart !== -1
      ? nextStart
      : source.indexOf('match__body-watch', blockStart);
    const blockHtml = blockEnd !== -1 ? source.slice(blockStart, blockEnd) : source.slice(blockStart);

    const name = cleanText(
      String(blockHtml).match(/<div\b[^>]*class=(["'])[^"']*\bteam__title\b[^"']*\1[^>]*>\s*<span[^>]*>([\s\S]*?)<\/span>/i)?.[2]
    );
    const logoMatch = String(blockHtml).match(/<i\b[^>]*data-theme-light=(["'])([^"']+)\1[^>]*>/i)
      || String(blockHtml).match(/<i\b[^>]*data-theme-dark=(["'])([^"']+)\1[^>]*>/i);
    const netWorthMatch = String(blockHtml).match(/networth[\s\S]*?<strong>([\s\S]*?)<\/strong>/i);
    const netWorth = netWorthMatch ? Number(cleanText(netWorthMatch[1])) : null;
    teams.push({
      name,
      logo: logoMatch?.[2] || null,
...
export function parseDltvLiveMatches(html) {
  ...
  for (const block of collectBlocks(html, LIVE_MATCH_BLOCK_RE)) {
    ...
    const parseScore = (raw) => {
      const rawKills = cleanText(String(raw || '').match(/<strong[^>]*>([\s\S]*?)<\/strong>/i)?.[1]);
      const kills = rawKills === '-' || rawKills === '' ? null : Number(rawKills);
      const wins = Number(cleanText(String(raw || '').match(/<small[^>]*>\((\d+)\)<\/small>/i)?.[1]) || 0);
      return { kills: Number.isFinite(kills) ? kills : null, wins };
    };
    ...
    live.push({
      ...
      radiantNetWorth: teams[0].netWorth ?? null,
      direNetWorth: teams[1].netWorth ?? null,
...
export function parseDltvFinishedMatches(html) {
  ...
  // DLTV 新版 results 页：同一赛事分组内只有第一场带 match__head-event 赛事头
  // （含 /events/ 链接），后续场次省略该头。向前继承上一个块的赛事名与赛事链接，
  // 否则这些场次会被 `if (!tournament) continue` 整体丢弃（表现为"某赛事只剩一场"）。
  let lastTournament = '';
  let lastEventUrl = null;

  for (const block of collectBlocks(html, FINISHED_MATCH_BLOCK_RE)) {
    const ownTournament = extractHeadTournament(block.html);
    const tournament = ownTournament || lastTournament;
    if (!tournament) continue;
    if (ownTournament) lastTournament = ownTournament;
    ...
    const ownEventUrl = extractEventUrl(block.html);
    const eventUrl = ownEventUrl || lastEventUrl;
    if (ownEventUrl) lastEventUrl = ownEventUrl;
