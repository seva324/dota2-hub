...
function extractTeams(html) {
  ...
  for (const block of teamBlocks.slice(0, 2)) {
    ...
    // 经济领先(networth)，仅 live 卡片存在
    const netWorthMatch = String(block[2] || '').match(/<div\b[^>]*class=(["'])[^"']*\bnetworth\b[^"']*\1[^>]*>\s*<i[^>]*>[\s\S]*?<strong>([\s\S]*?)<\/strong>/i);
    const netWorth = netWorthMatch ? Number(cleanText(netWorthMatch[2])) : null;
    teams.push({ name, logo: logoMatch?.[2] || null, netWorth: Number.isFinite(netWorth) ? netWorth : null });
  }
  return teams;
}
