...
function extractTeams(html) {
  ...
  for (const block of teamBlocks.slice(0, 2)) {
    ...
    teams.push({ name, logo: logoMatch?.[2] || null, netWorth: null });
  }

  // 独立提取 networth：按出现顺序取非空值，对应 radiant / dire
  const netWorthValues = [...String(html || '').matchAll(/<div\b[^>]*class=(["'])[^"']*\bnetworth\b[^"']*\1[^>]*>[\s\S]*?<strong>([\s\S]*?)<\/strong>/gi)]
    .map((match) => {
      const value = Number(cleanText(match[2]));
      return Number.isFinite(value) ? value : null;
    })
    .filter((value) => value !== null);

  if (teams[0] && netWorthValues[0] !== undefined) teams[0].netWorth = netWorthValues[0];
  if (teams[1] && netWorthValues[1] !== undefined) teams[1].netWorth = netWorthValues[1];
