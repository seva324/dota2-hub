...
function extractTeams(html) {
  // 按 team 块归属提取（每个 match__body-details__team 内找自己的 name/logo/networth），
  // 用三闭合 </div></div></div> 匹配完整块，避免 radiant/dire networth 归属错乱
  const teamBlocks = [...String(html || '').matchAll(/<div\b[^>]*class=(["'])[^"']*\bmatch__body-details__team\b[^"']*\1[^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/gi)];
  const teams = [];
