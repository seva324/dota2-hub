...
let ensurePlayerProfileCacheTablePromise = null;

export async function ensurePlayerProfileCacheTable(db) {
  if (ensurePlayerProfileCacheTablePromise) {
    return ensurePlayerProfileCacheTablePromise;
  }

  ensurePlayerProfileCacheTablePromise = (async () => {
    await db.query(`
    ...
    ensurePlayerProfileCacheTablePromise = null;
    throw error;
  });
...
async function fetchPlayerProfileAccountSupplementRows(db, accountId, options = {}) {
  const nowTs = Math.floor(Date.now() / 1000);
  const windowDays = Math.max(7, Math.min(365, Math.trunc(Number(options.windowDays || 180))));
  const minStartTime = nowTs - windowDays * 24 * 60 * 60;
  const matchLimit = Math.max(10, Math.min(80, Math.trunc(Number(options.matchLimit || 40))));
    ...
    `
      ...
      WHERE m.start_time <= $2
        ...
        AND EXISTS (
          ...
          WHERE COALESCE(player->>'account_id', player->>'accountId', player->>'accountid') = $1
        )
      ORDER BY m.start_time DESC
      LIMIT $4
    `,
    [String(accountId), nowTs, minStartTime, matchLimit]
  );
}
...
export async function buildPlayerProfilePayload(db, rawAccountId, options = {}) {
  ...
  const supplementedRows = options.fastMode === true
    ? []
    : await fetchPlayerProfileAccountSupplementRows(db, accountId, {
        windowDays: 180,
        matchLimit: 40,
      }).catch((error) => {
        console.warn('[PlayerProfileCache] Failed to load account supplement rows:', error?.message || error);
        return [];
      });
...
export async function getPlayerProfilePayload(db, rawAccountId, options = {}) {
  ...
  if (preferFast && !forceRefresh) {
    if (hasCachedPayload) {
      if (forceRefresh !== true && (partial || !fresh)) {
        void schedulePlayerProfileRefresh(db, accountId, {
      ...
      return {
        ...
        source: partial ? (fresh ? 'cache-partial' : 'stale') : (fresh ? 'cache' : 'cache-fallback'),
