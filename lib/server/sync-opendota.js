...
async function fetchExistingSeriesScores(db, seriesIds) {
  ...
  const normalized = seriesIds
    .map((id) => Math.trunc(Number(id)))
    .filter((id) => Number.isFinite(id));
  if (normalized.length === 0) return new Map();

  const rows = await db.query(
    `SELECT series_id::BIGINT AS series_id, radiant_team_id, dire_team_id, radiant_wins, dire_wins
     FROM series
...
async function refreshSeriesScores(db, seriesIds) {
  ...
  const rows = await db.query(
    `SELECT
       s.series_id::BIGINT AS series_id,
       COALESCE(SUM(CASE
     ...
     WHERE s.series_id = ANY($1::bigint[])
