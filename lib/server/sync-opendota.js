...
async function backfillMissingSeriesIds(db, matchIds = []) {
  ...
  const candidateSeriesRows = await db.query(
    `
      SELECT
...
export async function runSyncOpenDota() {
  ...
  try {
    ...
    const fallbackSeries = await backfillMissingSeriesIds(
      db,
      allMatches.map((match) => Number(match.match_id)).filter((id) => Number.isFinite(id))
    );
