...
// upcoming 的 match-page 每轮强制全热（ttlMs=0）：赛前列表动态变化，新比赛随时加入，
// 若按 fresh-skip 会漏掉新增项；upcoming 数量少（~10 场），每轮全量成本可控，
// 配合 30min 新鲜窗口 + 30min cron 间隔，用户点击几乎必命中 Neon。
const UPCOMING_FORCE_WARM = true;
...
// 本地跑（MacBook cron 直连 Neon，无平台 60s 限制）可传更大的 timeBudgetMs。
...
// 单个 match-page 预热的单页上限：EdgeOne 出口直连 dltv 常拿到壳页（无 series_item），
// 必须靠 jina 抓完整大页面（710KB-1.2MB），jina 实测 14-20s。10s 上限会把 jina 全部
// 掐断 → warm 每轮 warmed=0（正是观测到的现象）。放宽到 25s 让 jina 有机会完成；
// 通过 WARM_TIME_BUDGET_MS(30s) + 低并发(2) 控制整体，慢页超预算时下一轮收敛。
const PER_PAGE_WARM_MS = 25_000;
 ...
 * details 记录每页结果与抓取路径诊断（source/attempts），便于定位 EdgeOne
 * 出口抓取失败的具体环节（direct 壳页 / jina 缓存未命中 / jina 冷抓超时）。
 */
async function warmEntries({ db, entries, ttlMs, startedAt, deadline, timeBudgetMs, fetchImpl }) {
  let skippedFresh = 0;
  let budgetHit = false;
  const warmed = [];
  const details = [];
  if (!db || entries.length === 0) return { warmed, skippedFresh, budgetHit, details };
  ...
  await mapWithConcurrency(toWarm, WARM_CONCURRENCY, async (entry) => {
    if (Date.now() > deadline || Date.now() - startedAt > timeBudgetMs) {
      budgetHit = true;
      return null;
    }
    ...
    const ok = Boolean(outcome.result?.series);
    details.push({
      seriesId: entry.seriesId,
      ok,
      source: outcome.result?.source || (outcome.error ? 'timeout' : 'no-series'),
      attempts: (outcome.result?.attempts || []).map((a) => ({
        t: a.sourceType,
        s: a.status,
        b: a.bytes,
        ms: a.ms,
    ...
    if (ok) warmed.push(entry.seriesId);
    return ok ? entry.seriesId : null;
  });

  return { warmed, skippedFresh, budgetHit, details };
}
 ...
 */
async function warmStatsEntries({ db, entries, ttlMs, startedAt, deadline, timeBudgetMs, fetchImpl }) {
  let skippedFresh = 0;
  let budgetHit = false;
  const warmed = [];
...
export async function warmDltvCaches(options = {}) {
  ...
  // 本地跑（无平台 60s 上限）可放宽时间预算，一轮热完所有非 fresh 页。
  ...
  const upcomingWarm = await warmEntries({
    ...
    ttlMs: UPCOMING_FORCE_WARM ? 0 : COMPLETED_FRESH_MS,
    startedAt,
    deadline,
    timeBudgetMs,
    fetchImpl: options.fetchImpl,
