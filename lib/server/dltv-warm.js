 ...
 */
async function warmEntries({ db, entries, ttlMs, startedAt, fetchImpl }) {
  let skippedFresh = 0;
  let budgetHit = false;
  const warmed = [];
 ...
 */
async function warmStatsEntries({ db, entries, ttlMs, startedAt, fetchImpl }) {
  let skippedFresh = 0;
  let budgetHit = false;
  const warmed = [];
  if (!db || entries.length === 0) return { warmed, skippedFresh, budgetHit };
  ...
  await mapWithConcurrency(toWarm, WARM_CONCURRENCY, async (entry) => {
    ...
    const { stats } = await getDltvSeriesStats({ seriesId: entry.seriesId }, { forceRefresh: true, fetchImpl });
    if (stats) warmed.push(entry.seriesId);
    return stats ? entry.seriesId : null;
  });
