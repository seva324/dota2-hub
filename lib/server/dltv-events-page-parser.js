...
/* ------------------------------------------------------------------ */
/* /events：Ongoing 表格（HTML）—— DLTV 2026-08 起 ongoing 单列独立表格  */
/* ------------------------------------------------------------------ */

const ONGOING_SECTION_RE = /<section[^>]*class=["'][^"']*\bongoing__events\b[^"']*["'][^>]*>([\s\S]*?)<\/section>/gi;

/** 解析 ongoing 表格行（结构同 finished 表格：日期/名称/地区/类型/Tier/奖金）。 */
function parseOngoingTableRow(block, href) {
  ...
  // cell__text 顺序：地区、类型(Online/Offline)、Tier、奖金池 → 地区取第一个非类型/tier/奖金项
  const location = cellTexts.find((value) => value
    && !/\btier\b/i.test(value)
    && !/^\$[\d,]/.test(value)
    && !/^(online|offline)$/i.test(value)) || null;

  const logoMatch = String(block || '').match(/cell__logo[^>]*data-theme-light=(["'])([^"']+)\1/i);
...
function parseDltvOngoingTable(raw) {
  const source = String(raw || '');
  const entries = [];
  for (const section of source.matchAll(ONGOING_SECTION_RE)) {
    for (const match of section[1].matchAll(TABLE_ROW_RE)) {
      const entry = parseOngoingTableRow(match[4], match[2]);
...
export function parseDltvOngoingUpcomingPage(raw) {
  ...
  // 去重（featured + 列表卡片重复）；同一赛事在 ongoing 表格出现时以表格状态为准
  const seen = new Map();
  for (const entry of [...cardEntries, ...parseDltvOngoingTable(source)]) {
    const key = `${entry.title}|${entry.startTime || ''}|${entry.endTime || ''}`;
    const existing = seen.get(key);
    if (!existing || (existing.status !== 'ongoing' && entry.status === 'ongoing')) {
      seen.set(key, entry);
    }
  }
  return [...seen.values()];
