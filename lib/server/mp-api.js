...
export async function loadTeamMap(db, teamIds = null) {
  ...
  const rows = normalizedTeamIds.length
    ? await db`
        SELECT *
        FROM teams
...
export async function loadTeamDetailPayload(db, teamId, { limit, offset, publicOrigin = '' }) {
  ...
  const selectedTeamRows = await db`
    SELECT *
    FROM teams
    WHERE team_id = ${String(teamId)}
    LIMIT 1
  `;
  const selectedTeam = selectedTeamRows[0] || null;
