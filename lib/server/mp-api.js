...
export async function loadTournamentDetailPayload(db, tournamentId, { limit, offset, req = null }) {
...
export async function loadHomePayload(db, options = {}) {
  ...
  const { req = null, ...newsOptions } = requestOptions;
  const [tournamentsResult, upcomingResult, news, liveMatches] = await Promise.all([
    ...
    loadStoredNewsSummaries(db, 6, newsOptions),
