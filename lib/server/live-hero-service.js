...
export async function getLiveHeroPayloads(db, options = {}) {
  ...
  const cachedRows = filterRowsByTargetTeamKey(
  ...
  try {
    ...
    if (!snapshots.length) {
      const fallbackRows = options.forceRefresh
        ? []
        : filterRowsByAllowedLeagueNames(
            cachedRows,
            new Set(leagueMatchedCandidates.map((row) => normalizeLeagueName(row.leagueName)).filter(Boolean)),
          );
      return fallbackRows.map(rowToApiPayload).filter(Boolean);
    }
    ...
    return mergedRows.map(rowToApiPayload).filter(Boolean);
  } catch (error) {
