...
export function getBo3ImageFetchCandidates(rawUrl, baseUrl = 'https://bo3.gg') {
  ...
  try {
    ...
    if (host === 'image-proxy.bo3.gg') {
      // files.bo3.gg is the stable origin; image-proxy.bo3.gg intermittently
      // times out or serves a Cloudflare challenge, so try the origin first.
      appendUnique(candidates, `https://files.bo3.gg${pathWithoutProxySuffix}`);
      appendUnique(candidates, url.toString());
      appendUnique(candidates, `https://wsrv.nl/?url=${encodeURIComponent(normalized)}`);
      return candidates;
    }
