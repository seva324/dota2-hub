...
function parseSingleSlide(slideHtml) {
  ...
  const prize = cleanText(
    String(slideHtml).match(/prize\s*pool[\s\S]*?<div\b[^>]*class=(["'])[^"']*\bcell__name\b[^"']*\1[^>]*>([\s\S]*?)<\/div>/i)?.[2]
  ) || null;

  const tier = cleanText(
    String(slideHtml).match(/>tier<\/div>[\s\S]*?<div\b[^>]*class=(["'])[^"']*\bcell__name\b[^"']*\1[^>]*>([\s\S]*?)<\/div>/i)?.[2]
  ) || null;
