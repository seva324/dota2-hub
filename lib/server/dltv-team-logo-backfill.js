...
function buildExistingDltvLogoMap(rows) {
  const byKey = new Map();

  for (const row of rows) {
    if (!isUsableDltvTeamLogo(row.logo_url)) continue;
    for (const key of buildDltvLookupKeys(buildTeamCandidates(row))) {
      if (!byKey.has(key)) byKey.set(key, row.logo_url);
    }
  }
...
function resolveExistingDltvLogo(byKey, team) {
  for (const key of buildDltvLookupKeys(buildTeamCandidates(team))) {
...
export async function backfillDltvTeamLogos(db, options = {}) {
  ...
  const existingDltvLogos = buildExistingDltvLogoMap(rows);
  const updates = [];
  let matched = 0;
  let alreadyDltv = 0;

  for (const row of candidates) {
    const dltvLogo = resolveExistingDltvLogo(existingDltvLogos, row)
      || resolveDltvLogoForTeam(index, row);
    if (!dltvLogo) continue;
    matched += 1;

    if (!shouldUpdateLogo(row.logo_url, dltvLogo)) {
      alreadyDltv += 1;
      continue;
    }
    ...
    if (!options.dryRun) {
      await db`
        UPDATE teams
        SET logo_url = ${dltvLogo}, updated_at = NOW()
