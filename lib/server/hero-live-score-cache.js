...
export async function findHeroLiveScoreBySourceSeriesId(db = getDb(), seriesId) {
  if (!db || !seriesId) return null;
  await ensureHeroLiveScoresTable(db);
  const rows = await db.query(
    `SELECT * FROM hero_live_scores WHERE source_series_id = $1 ORDER BY last_seen_at DESC LIMIT 1`,
    [String(seriesId)]
  );
  return rows[0] || null;
}
