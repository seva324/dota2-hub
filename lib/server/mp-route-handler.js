...
export async function handleMpRoute(req, res, db) {
  ...
  try {
    switch (resource) {
      case 'home': {
        const payload = await loadHomePayload(db, { req, publicOrigin });
      ...
      case 'upcoming': {
        ...
        const payload = await loadUpcomingPayload(db, { days, limit, offset, publicOrigin });
        sendApiSuccess(res, payload);
        return true;
      }
      case 'tournament': {
        ...
        const payload = await loadTournamentDetailPayload(db, id, { limit, offset, req, publicOrigin });
        if (!payload) {
      ...
      case 'team': {
        ...
        const payload = await loadTeamDetailPayload(db, id, { limit, offset, publicOrigin });
        if (!payload) {
