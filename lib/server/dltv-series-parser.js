...
// DLTV 位置码：1=Core, 2=Mid, 3=Offlane, 4=Support, 5=Full Support → 一号位~五号位。
...
export function parseDltvSeriesItem(html) {
  ...
  return {
    ...
    // 已结束时间：非空 = 比赛已打完（DLTV status 2=finished）。未结束的 series 不允许进 6h 长缓存。
    endedAt: raw.ended_at ? Math.floor(new Date(raw.ended_at).getTime() / 1000) : null,
