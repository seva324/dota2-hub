...
export async function fetchMatchPageHtml(url, fetchImpl = fetch, timeoutMs = PAGE_FETCH_TIMEOUT_MS) {
  ...
  const finalize = (result) => {
    ...
    fetchDiagnostics.set(url, summary);
    return { ...result, attempts: summary };
  };
  ...
  if (direct.raw) return finalize({ ...direct, sourceUrl: url });
  ...
  if (cached.raw) return finalize({ ...cached, sourceUrl: jinaUrl });
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    attempts.push(jina);
    if (jina.raw) return finalize({ ...jina, sourceUrl: jinaUrl });
  }

  return finalize({ raw: '', sourceType: 'failed', sourceUrl: url });
}
...
// 最近一次抓取诊断（url → attempts 摘要）：冷抓超时（Promise.race 落败）时
// 底层抓取仍在后台跑，结果拿不到，但 attempt 进度已记录，供 API 输出定位失败环节。
const fetchDiagnostics = new Map();
...
export async function getDltvMatchPage({ seriesId, slug }, options = {}) {
  ...
  try {
    ...
    // 超时：底层抓取仍在后台跑，诊断信息从最近一次 attempt 进度取。
    const url = buildDltvMatchPageUrl(seriesId, slug);
    return { series: null, source: 'timeout', attempts: (url && fetchDiagnostics.get(url)) || [] };
  }
}
