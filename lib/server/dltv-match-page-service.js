...
// 冷抓 + slug 重建可能两段各 5-8s，总超时若只有 15s 极易触发 15s 超时 → match-page 空报、
// 前端"暂无比赛数据"。放宽到 25s：jina 单段罕见 10s+ 也留足裕量，同时前端无客户端超时，
// 串行等待 25s 可接受，且成功后写入内存/Neon，后续点击立即命中。
const MATCH_PAGE_TOTAL_TIMEOUT_MS = 25000;
...
export async function fetchMatchPageHtml(url, fetchImpl = fetch, timeoutMs = PAGE_FETCH_TIMEOUT_MS) {
  const attempts = [
    { url, type: 'direct', timeoutMs, headers: {} },
    {
      ...
      type: 'jina',
      // jina 从 EdgeOne 出口实测 5-9s（dltv direct 被反爬拦截时），预算放宽到 10s。
      timeoutMs: 10000,
