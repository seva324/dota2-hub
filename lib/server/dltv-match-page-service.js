...
async function fetchMatchPageHtml(url, fetchImpl = fetch) {
  const attempts = [
  ...
  for (const attempt of attempts) {
    ...
    try {
      ...
      // 详情页必须包含 series_item 才算成功。
      // 注意：不带 slug 的 /matches/<seriesId> 会返回 404，但页面仍内嵌 series_item，
      // 所以这里不依赖 res.ok，只看内容里有没有 series_item。
      if (String(text || '').includes('series_item')) {
