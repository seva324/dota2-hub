    ...
    .map((columnHtml, columnIndex) => {
      ...
      const matches = columnHtml
        ...
        .map((seriesHtml, matchIndex) => {
          const teams = [...seriesHtml.matchAll(/<div class="col__serie-teams__item"([\s\S]*?)<\/div>\s*<\/div>/g)]
            .map((match) => {
              const teamHtml = match[1] || '';
              return {
                name: cleanText(teamHtml.match(/<div class="name [^"]*">([^<]+)<\/div>/)?.[1]) || 'TBD',
          ...
          const dateParts = [...seriesHtml.matchAll(/<div[^>]*>\s*([^<]*)\s*<\/div>/g)].map((match) => cleanText(match[1])).filter(Boolean);
          const startTime = dateParts.length >= 2 ? `${dateParts[0]} ${dateParts[1]}`.trim() : (dateParts[0] || null);
