...
const FEATURED_TOURNAMENT_CACHE_TTL_MS = 2 * 60 * 1000;
...
const FEATURED_TOURNAMENT_DEFINITIONS = [
  ...
  },
  {
    ...
    leagueId: '19532',
    names: ['DreamLeague Division 2 Season 4'],
    sourceLabel: 'DLTV',
  ...
  },
  {
    ...
    leagueId: '19130',
    names: ['ESL Challenger China Season 3'],
    sourceLabel: 'DLTV',
...
function parseRoundRobinGroups(groupSectionHtml) {
  const markers = [...String(groupSectionHtml || '').matchAll(/<div class="table__head-item[^"]*big-text[^"]*">([\s\S]*?)<\/div>/g)];
...
function normalizePlayoffRoundKey(roundName) {
...
function normalizePlacementLabel(label) {
  return cleanText(label).toLowerCase().replace(/\s+/g, ' ');
}

function getFallbackPlacementPrizes(definition) {
  if (definition?.id === 'pgl-wallachia-s7' || definition?.id === 'pgl-wallachia-s8') {
    return [
      { placement: '1st Place', prize: '$300,000' },
      { placement: '2nd Place', prize: '$175,000' },
      { placement: '3rd Place', prize: '$120,000' },
      { placement: '4th Place', prize: '$80,000' },
      { placement: '5th - 6th Place', prize: '$60,000' },
      { placement: '7th - 8th Place', prize: '$40,000' },
    ];
  }
  if (definition?.id === 'dreamleague-division-2-season-4') {
    return [
      { placement: '1st Place', prize: '$15,000' },
      { placement: '2nd Place', prize: '$10,000' },
      { placement: '3rd Place', prize: '$8,000' },
      { placement: '4th Place', prize: '$7,000' },
      { placement: '5th - 6th Place', prize: '$4,000' },
      { placement: '7th - 8th Place', prize: '$1,000' },
    ];
  }
  if (definition?.id === 'esl-challenger-china-season-3') {
    return [
...
function parsePlacementPrizes(playoffsHtml, definition) {
  const fromDom = [...String(playoffsHtml || '').matchAll(/<div class="col__prize[^"]*">\s*<span>([\s\S]*?)<\/span>\s*<strong>([\s\S]*?)<\/strong>\s*<\/div>/g)]
    .map((match) => ({
      placement: cleanText(match[1]),
      prize: cleanText(match[2]),
    }))
    .filter((entry) => entry.placement && entry.prize);

  const merged = new Map(fromDom.map((entry) => [normalizePlacementLabel(entry.placement), entry]));
  for (const fallback of getFallbackPlacementPrizes(definition)) {
    if (!merged.has(normalizePlacementLabel(fallback.placement))) {
      merged.set(normalizePlacementLabel(fallback.placement), fallback);
    }
  }
...
function parseStandings(groupSectionHtml, definition) {
  if (definition?.format === 'double-elimination') {
    return {
...
function parsePlayoffSeriesMatches(columnHtml, roundName, columnIndex = 0) {
  return String(columnHtml || '')
    .split('<div class="col__serie ')
    .slice(1)
    .map((seriesHtml, matchIndex) => {
      const teams = [...seriesHtml.matchAll(/<div class="col__serie-teams__item"[^>]*>([\s\S]*?)<\/div>\s*(?=<div class="col__serie-teams__delimiter">|<div class="col__serie-teams__item"|<\/div>\s*<\/a>)/g)]
        .map((match) => {
          const teamHtml = match[1] || '';
          return {
            name: cleanText(teamHtml.match(/<div class="name [^"]*">([\s\S]*?)<\/div>/)?.[1]) || 'TBD',
            logoUrl: getCellLogoUrl(teamHtml),
            score: cleanText(teamHtml.match(/<div class="score[^>]*">\s*([^<]*)\s*<\/div>/)?.[1]) || '-',
          };
        })
        .filter(Boolean);
...
function parsePlayoffRounds(playoffsHtml) {
  const rounds = String(playoffsHtml || '')
    ...
    .map((columnHtml, columnIndex) => {
      ...
      return {
        roundName,
        roundKey: normalizePlayoffRoundKey(roundName),
        matches: parsePlayoffSeriesMatches(columnHtml, roundName, columnIndex),
      };
    })
    .filter(Boolean);

  const grandFinal = rounds.find((round) => /Grand Final/i.test(round.roundName));
  if (grandFinal && grandFinal.matches.length === 0) {
    const finalColumnHtml = String(playoffsHtml || '').match(/<div class="playoffs__box-row__col final[^"]*">([\s\S]*?)(?=<div class="col__prize gold">|<\/section>)/)?.[1]
      || String(playoffsHtml || '').match(/<div class="playoffs__box-row__col final[^"]*">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/section>/)?.[1]
      || String(playoffsHtml || '').match(/<div class="playoffs__box-row__col final[^"]*">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/)?.[1]
      || '';
    const finalMatches = parsePlayoffSeriesMatches(finalColumnHtml, grandFinal.roundName, rounds.length - 1);
    if (finalMatches.length > 0) {
      grandFinal.matches = finalMatches;
    }
  }
...
export function parseDltvFeaturedTournamentPage(html, definition) {
  ...
  const playoffs = {
    ...
    placementPrizes: parsePlacementPrizes(playoffsSectionHtml, definition),
  };
...
const featuredTournamentCache = new Map();
const featuredTournamentInFlight = new Map();

function getCachedFeaturedTournament(definition) {
  const cached = featuredTournamentCache.get(definition.id);
  if (!cached || cached.expiresAt <= Date.now()) {
    if (cached) featuredTournamentCache.delete(definition.id);
    return null;
  }
  return cached.payload;
}

export async function fetchFeaturedTournamentPayload(tournamentId, options = {}) {
  const definition = resolveFeaturedTournamentDefinition(tournamentId);
  if (!definition) return null;
  const forceRefresh = options?.forceRefresh === true;

  if (!forceRefresh) {
    const cachedPayload = getCachedFeaturedTournament(definition);
    if (cachedPayload) return cachedPayload;

    const pendingRequest = featuredTournamentInFlight.get(definition.id);
  ...
  const requestPromise = (async () => {
    ...
    try {
      ...
      const payload = parseDltvFeaturedTournamentPage(html, definition);
      featuredTournamentCache.set(definition.id, {
        payload,
        expiresAt: Date.now() + FEATURED_TOURNAMENT_CACHE_TTL_MS,
      });
      return payload;
    } finally {
  ...
  if (!forceRefresh) {
    featuredTournamentInFlight.set(definition.id, requestPromise);
  }

  try {
    return await requestPromise;
  } finally {
    featuredTournamentInFlight.delete(definition.id);
  }
}
