...
function parseStandingTable(tableHtml, defaultName) {
  ...
  for (const rm of tableHtml.matchAll(rowRe)) {
    ...
    // 列布局由表头决定（兼容 DLTV 中英双语表头）：
    //  - 双栏分组（Series/Maps/Points）：[rank, team, record, maps, points]
    //  - 单循环（Maps/Points，无 Series，中间可能留空列）：[rank, team, (空), maps, points]
    //  - 瑞士轮（Matches/比赛）：[rank, team, record]
    const hasSeries = /series|战绩|系列|对局/i.test(heads.join(' '));
    const hasMaps = /maps|图/i.test(heads.join(' '));
    const isSwiss = /matches|对阵|比赛/i.test(heads.join(' ')) && !hasSeries && !hasMaps;
 ...
 *  返回 [{ html, start }]：start 为该表在 block 中的起始偏移（用于就近取 card__title 命名阶段块）。 */
function splitTables(block) {
  ...
  while (true) {
    ...
    tables.push({ html: block.slice(start, chunkEnd), start });
    cursor = chunkEnd;
  }
  return tables;
}
...
/* ------------------------------------------------------------------ */
/* stage match list（group__stage 内的比赛列表，如 TI2026 Elimination Round） */
/* ------------------------------------------------------------------ */

/**
 * 解析比赛列表型阶段块（如 Elimination Round）：表格行是 <a class="table__body-row"> 链接到
 * /matches/...，非积分榜行。行结构：左队名 cell__name → 中心（时间或比分）→ 右队名 cell__name，
 * 比分行中心格为 <div class="score">，时间行中心格为 data-moment="HH:mm"。
 */
function parseStageMatchList(tableHtml, teamBySlug) {
  const rows = [];
  const rowRe = /<a href="(https:\/\/dltv\.org\/matches\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/g;
  for (const rm of tableHtml.matchAll(rowRe)) {
    const row = rm[2];
    const names = [...row.matchAll(/class="cell__name">([\s\S]*?)<\/div>/g)].map((x) => cleanText(x[1]));
    if (names.length < 2) continue;
    const logoOf = (block) => {
      const raw = block.match(/data-theme-dark="([^"]+)"/)?.[1];
      return raw ? toAbsoluteAssetUrl(raw) : null;
    };
    const leftRaw = row.match(/class="cell__name">([\s\S]*?)<\/div>/)?.[1];
    const leftIdx = row.indexOf('class="cell__name"');
    const rightIdx = row.lastIndexOf('class="cell__name"');
    const leftTag = cleanText(leftRaw);
    const rightTag = cleanText(names[names.length - 1]);
    const scoreMatch = row.match(/<div class="score">([\s\S]*?)<\/div>/);
    const center = scoreMatch
      ? cleanText(scoreMatch[1]).replace(/\s+/g, ' ')
      : row.match(/data-moment="HH:mm">([^<]+)</)?.[1] || '';
    const [leftSlug, rightSlug] = matchUrlTeamSlugs(rm[1], teamBySlug);
    const leftCell = row.slice(0, leftIdx >= 0 ? leftIdx : 0);
    const rightCell = rightIdx >= 0 ? row.slice(rightIdx) : '';
    rows.push({
      url: rm[1],
      time: row.match(/data-event-matches-odd="([^"]+)"/)?.[1] || null,
      left: leftSlug && teamBySlug.get(leftSlug) ? teamBySlug.get(leftSlug) : leftTag,
      leftSlug,
      leftLogo: logoOf(leftCell),
      center,
      isLive: /label__danger">\s*live/i.test(row),
      right: rightSlug && teamBySlug.get(rightSlug) ? teamBySlug.get(rightSlug) : rightTag,
      rightSlug,
      rightLogo: logoOf(rightCell),
    });
  }
  return rows;
}

function parseGroupStage(html, teamBySlug) {
  const groupStart = html.indexOf('<section class="group__stage">');
  ...
  // 收集区块内 card__title（阶段名，如 "Elimination Round"），供比赛列表型阶段块命名。
  const titleSpans = [...block.matchAll(/<div class="card__title">([\s\S]*?)<\/div>/g)]
    .map((m) => ({ text: cleanText(m[1]), at: m.index }));

  for (const { html: tableHtml, start: tableStart } of splitTables(block)) {
    const parsed = parseStandingTable(tableHtml, '');
    // 比赛列表型阶段块（如 TI2026 Elimination Round）：表格行是 /matches/ 链接而非积分榜行，
    // 解析为 { name, matches } 组，前端单独渲染成阶段比赛列表。
    if (parsed.rows.length === 0) {
      const matches = parseStageMatchList(tableHtml, teamBySlug);
      if (matches.length > 0) {
        const title = [...titleSpans].reverse().find((t) => t.at < tableStart);
        groups.push({ name: title ? title.text : `Stage ${groups.length + 1}`, matches });
      }
      continue;
    }
  ...
  // 瑞士轮检测：区块内存在「R 1 … R 「n」」对阵表头 → 该赛事小组赛为瑞士轮，
  // 前端据此在积分榜右侧按时间顺序渲染每个队的「对阵结果」。
  // 只作用于积分榜组（rows），比赛列表型阶段块（matches）不参与。
  const swissRounds = detectSwissRoundHeaders(block);
  if (swissRounds.length > 0) {
    for (const g of groups) {
      if (g.rows?.length) g.rounds = swissRounds;
    }
  }
  return groups;
}

/** 识别瑞士轮「轮次列」数量，返回规范化的轮名数组（如 ['R1',…,'R6']）。
 *
 *  不依赖轮头文本是否为 "R N" —— 生产抓取的 dltv HTML 里轮头 label 可能是
 *  "R 1"、轮次图标、日期等不同写法，但右侧对阵表格的第二块 col-6 的
 *  table__head-item 数量恒定等于轮数。可靠做法：
 *   1) 取组区块内第二块 col-6(对阵/轮次表)
 *   2) 数其 table__head-item 个数 = 轮数
 *   3) 有 ≥1 轮列 → 属瑞士轮，返回 ['R1'..'RN']; 否则返回空(非瑞士/循环赛)
 */
function detectSwissRoundHeaders(block) {
  const col6 = [...block.matchAll(/class="col-6"/g)];
  if (col6.length < 2) {
    // 无第二块(对阵)列表：不在 group 段内的轮头文本回落(兼容单块但内嵌轮头)
    return detectSwissRoundHeadersByText(block);
  }
  const fixtureStart = col6[1].index;
  const fixtureEnd = col6.length > 2 ? col6[2].index : block.length;
  const fixtureBlock = block.slice(fixtureStart, fixtureEnd);
  const headItems = fixtureBlock.match(/class="table__head-item/g) ?? [];
  // 可能的夹具轮列: 排除【团队/比赛】之类 1-2 个元列后取真实轮数
  const roundCols = Math.max(0, headItems.length);
  if (roundCols < 1) return detectSwissRoundHeadersByText(block);
  return buildRoundNames(roundCols);
}

/** 文本回落：从组区块里的 "R N" 轮头文本提取(仍保留，兼容旧 HTML)。 */
function detectSwissRoundHeadersByText(block) {
  const roundNames = [];
  const seen = new Set();
  const addRound = (raw) => {
    const norm = String(raw || '').replace(/\s+/g, '').toUpperCase();
    if (/^R\d+$/.test(norm) && !seen.has(norm)) {
      seen.add(norm);
      roundNames.push(norm);
    }
  };
  for (const m of block.matchAll(/table__head-item[^>]*>\s*(R\s*\d+)\s*<\/div>/gi)) addRound(m[1]);
  for (const m of block.matchAll(/table__head-item[^>]*>[\s\S]{0,30}?(R\s*\d+)[\s\S]{0,10}?(?:<\/div>|<\/span>)/gi)) addRound(m[1]);
  if (roundNames.length === 0) {
    for (const m of block.matchAll(/>\s*(R\s*[1-9]\d*)\s*<\/?(?:div|span)>|>?\s*(R\s*[1-9]\d*)\s*<\/?(?:div|span)>?/gi)) addRound(m[1] || m[2]);
  }
  return roundNames.sort((a, b) => Number(a.replace(/\D/g, '')) - Number(b.replace(/\D/g, '')));
}

/** 生成 ['R1',…,'RN']。 */
function buildRoundNames(count) {
...
function parseMatchRows(block, teamBySlug) {
  ...
  for (const m of block.matchAll(/<a href="(https:\/\/dltv\.org\/matches\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/g)) {
    const row = m[2];
    // DLTV 在 <a class="table__body-row"> 上带 data-event-matches-odd="YYYY-MM-DD HH:MM:SS"（已结束=完赛时间，未开赛=开赛时间）。
    const oddRaw = m[0].match(/data-event-matches-odd="([^"]+)"/)?.[1];
    const time = oddRaw && /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(oddRaw) ? oddRaw : null;
    const cells = [];
    let cc = 0;
    while (true) {
...
/** 中心格是比分（"2 - 1"/"FF - W"）而非开赛时间（"2026-08-07 09:00:00"）→ 已结束；live 行除外。 */
function isFinishedCenter(center) {
  return Boolean(String(center || '').trim()) && !/\d{4}-\d{2}-\d{2}/.test(String(center || ''));
}
...
function parseMatches(html, teamBySlug) {
  ...
  // 按行内容分类（开赛时间 vs 比分）+ 按 match URL 去重，不依赖 "Finished matches" 文本标记：
  // 生产环境抓到的 HTML 有时匹配不到该标记（finishedIdx=-1），已结束比赛会整段误入 upcoming；
  // 且 DLTV 会重复渲染一份表格，直接切片会产出重复行。
  ...
  const rows = [...parseMatchRows(mainBlock, teamBySlug), ...parseMatchRows(finishedBlock, teamBySlug)];
  const matches = [];
  ...
  for (const row of rows) {
    ...
    seen.add(row.url);
    if (row.isLive || !isFinishedCenter(row.center)) matches.push(row);
    else finishedMatches.push(row);
  }
  return { matches, finishedMatches };
}
...
export function parseDltvEventDetailPage(raw, slug) {
  ...
  // 阶段比赛列表（Elimination Round）的行需要 slug→全名 映射，但参赛队伍已含全部 slug，
  // 先建 participants-only 映射供 parseGroupStage 使用，之后再合并积分榜行的补充映射。
  const groups = parseGroupStage(html, buildTeamBySlug(participants, []));
