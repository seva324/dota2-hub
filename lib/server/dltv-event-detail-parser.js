...
/** 识别瑞士轮表头（如 "R 1"…"R 6"），返回规范化的轮名数组（如 ['R1',…,'R6']）。
 *  生产抓取的 HTML 结构可能有多种包裹形态，这里做多层兜底：
 *   1) table__head-item 直接文本（`>R 1</div>`）
 *   2) table__head-item 多包一层 span/div
 *   3) 组区块内任意 `>R <数字></div|span>` 表头文本（jina 重排后）
 *   4) 组区块内纯文本列头 `>R 1<`/`R 1 `（属性/换行差异）
 */
