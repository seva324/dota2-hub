 ...
 *  兼容直接文本 `>R 1<` 与表头项内多一层嵌套(span/div)两种结构。 */
function detectSwissRoundHeaders(block) {
  ...
  // 表头项内直接是文本（`...>R 1</div>`）或多包一层（`...>R 1</span></div>`）
  const re = /table__head-item[^>]*>([\s\S]{0,40}?)(?:<\/div>|<\/span>\s*<\/div>|<\/span>)/gi;
  for (const m of block.matchAll(re)) {
    addRound(m[1]);
  }
  // 兜底：组区块里任何形如 `>R <数字><` 的表头文本（如 jina 重排后的结构）
  if (roundNames.length === 0) {
    for (const m of block.matchAll(/>\s*(R\s*\d+)\s*<\/?(?:div|span)>/gi)) addRound(m[1]);
  }
