...
function parseOverview(html) {
  ...
  for (const m of block.matchAll(/<div class="item__title">([\s\S]*?)<\/div>[\s\S]*?<div class="item__text">([\s\S]*?)<\/div>\s*<\/div>/g)) {
    overview[cleanText(m[1])] = cleanText(m[2]);
  }
...
function parseMatches(html) {
  const block = html.match(/<div class="matches__scores-table">([\s\S]*?)(?=<section class="event__participants">)/)?.[1] || '';
  const finishedMark = '<div class="card__title mt-4">Finished matches</div>';
  const finishedIdx = block.indexOf(finishedMark);
  const mainBlock = finishedIdx >= 0 ? block.slice(0, finishedIdx) : block;
  const finishedBlock = finishedIdx >= 0 ? block.slice(finishedIdx) : '';
  return {
...
function parsePrizePool(html) {
  ...
  const prizes = [];
  const seen = new Set();
  for (const pm of block.matchAll(/class="event__prizepool-block ([a-z]+)"[\s\S]*?<div class="title[^"]*">([\s\S]*?)<\/div>[\s\S]*?<div class="prize[^"]*">([\s\S]*?)<\/div>[\s\S]*?<strong[^>]*>([\s\S]*?)<\/strong>[\s\S]*?<span>Place<\/span>/g)) {
    const key = `${pm[4]}|${pm[3]}`;
    if (seen.has(key)) continue;
    seen.add(key);
    prizes.push({ tone: pm[1], team: cleanText(pm[2]), prize: cleanText(pm[3]), place: cleanText(pm[4]) });
  }
  return prizes;
}
