  ...
  if (!redisClientPromise) {
    redisClientPromise = (async () => {
      ...
      client.on('error', (error) => {
        ...
        console.error('[dltv-matches-hot-cache] Redis error:', lastRedisError);
      });
      ...
      lastRedisError = error instanceof Error ? error.message : String(error);
      console.error('[dltv-matches-hot-cache] Redis connect failed:', lastRedisError);
      return null;
    });
  }
...
/** 诊断：Redis 是否可达（预热 cron 用，排查列表缓存为何未共享）。 */
export async function isDltvRedisAvailable() {
  const client = await getRedisClient();
  ...
  try {
    ...
    console.error('[dltv-matches-hot-cache] Redis ping failed:', error instanceof Error ? error.message : String(error));
    return false;
  }
}

/** 详细诊断：区分 REDIS_URL 未设 / 连不上，并带上主机与错误信息（不暴露密码）。 */
export async function inspectDltvRedis() {
  ...
  if (!rawUrl) {
    return { envSet: false, host: '', reachable: false, error: 'REDIS_URL not set' };
  }
  const client = await getRedisClient();
  if (!client) {
    return { envSet: true, host, reachable: false, error: lastRedisError || 'connect failed (see logs)' };
  }
  try {
    await client.ping();
    return { envSet: true, host, reachable: true };
  } catch (error) {
    return { envSet: true, host, reachable: false, error: error instanceof Error ? error.message : String(error) };
  }
}
