...
const DLTV_CATALOG_DEMO_GROUPS = new Set(['blast-slam-7', 'dreamleague-season-29']);
...
function summarizeAggregateStatus(events) {
  const statuses = new Set(events.map((event) => String(event?.status || '').toLowerCase()).filter(Boolean));
  if (statuses.has('ongoing')) return 'ongoing';
  if (statuses.has('upcoming')) return 'upcoming';
  if (statuses.size > 0 && [...statuses].every((status) => status === 'finished')) return 'finished';
  return null;
}
...
function buildAggregateTournamentMetadata(rule, events, metadataByUrl) {
  ...
  return {
    ...
    status: summarizeAggregateStatus(events),
    tier,
...
async function resolveTournamentRecord(db, cache, tournamentName, eventMeta) {
  ...
  const explicitCandidate = mapping?.leagueId ? findTournamentCandidateByLeagueId(cache, mapping.leagueId) : null;
  ...
  if (!shouldUseAggregateMapping) {
    await upsertTournamentMetadata(
      db,
      cache,
      leagueId,
