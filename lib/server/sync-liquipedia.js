...
import { resolveDltvLogoForTeam } from './dltv-team-logo-backfill.js';
...
const DLTV_CATALOG_DEMO_GROUPS = new Set([
  'blast-slam-7',
...
const DEMO_TOURNAMENT_MAPPINGS = [
  {
    key: 'dreamleague-division-2-season-4',
    eventSlug: 'dreamleague-division-2-season-4',
    leagueId: 19532,
    canonicalName: 'DreamLeague Division 2 Season 4',
  },
  {
    key: 'pgl-wallachia-s8-main',
    eventSlug: 'pgl-wallachia-season-8',
    leagueId: 19543,
    canonicalName: 'PGL Wallachia Season 8',
  },
  {
...
async function saveTeamFromOpenDota(db, teamName, dltvRankingIndex = null) {
  ...
  try {
    ...
    if (team?.team_id) {
      ...
      const dltvLogo = curatedLogo || resolveDltvLogoForTeam(dltvRankingIndex, {
        team_id: team.team_id,
        name: team.name || teamName,
        tag: team.tag,
      });
...
async function loadRelevantTeams(db, upcomingMatches) {
  const names = Array.from(new Set(
    (upcomingMatches || [])
      .flatMap((match) => [normalizeName(match?.radiantName), normalizeName(match?.direName)])
      .filter(Boolean)
  ));
  ...
  return db.query(
    `SELECT team_id, name, tag FROM teams
      WHERE LOWER(name) = ANY($1::text[])
         OR LOWER(COALESCE(tag, '')) = ANY($1::text[])`,
    [names]
  );
}
 ...
 */
async function fetchDltvUpcomingMatches(options = {}) {
  const mergeSources = options?.mergeSources !== false;
  const attempts = [
  ...
  for (const attempt of attempts) {
    ...
    try {
      ...
      if (upcoming.length > 0) {
        ...
        if (!mergeSources) {
          return {
            upcoming: upcoming.sort((left, right) => left.timestamp - right.timestamp),
            debug,
          };
        }
...
function normalizeSyncPhase(phase) {
  const normalized = String(phase || 'full').trim().toLowerCase();
  if (['fast', 'metadata', 'upcoming'].includes(normalized)) return normalized;
  return 'full';
}

export async function runSyncLiquipedia(options = {}) {
  ...
  try {
    ...
    const explicitSeedUrls = Array.from(new Set(
      (Array.isArray(options?.seedUrls) ? options.seedUrls : [options?.seedUrls])
        .map((value) => String(value || '').trim())
        .filter(Boolean)
    ));
    const syncUpcoming = phase !== 'metadata';
    const syncCatalogMetadata = phase !== 'upcoming';
    const includeUpcomingEventMetadata = phase === 'full';
    const mergeUpcomingSources = phase === 'full';

    await ensureUpcomingSeriesColumns(db);
    await ensureTournamentColumns(db);

    const dltvRankingIndex = null;
    const dltvLogoBackfill = {
      skipped: true,
      reason: 'deferred_to_backfill_dltv_team_logos_action',
    };
    ...
    if (syncUpcoming) {
      ...
      // Step 2b: Remove reschedule duplicates.
      // When DLTV reschedules a match the old code created a new row (id included the
      // timestamp) while the previous row with the stale time remained.  Delete the
      // stale rows: for every series_id that appears more than once keep only the row
      // with the most-recent updated_at (i.e. the row written by the most recent sync).
      await db`
      ...
      `;
      console.log('[Sync Liquipedia] Cleaned up reschedule duplicates');
      ...
      try {
        const result = await withTimeout(
          () => fetchDltvUpcomingMatches({ mergeSources: mergeUpcomingSources }),
          20000,
          'DLTV upcoming matches'
        );
    ...
    if (syncCatalogMetadata) {
      if (explicitSeedUrls.length > 0) {
        dltvCatalogSeeds = explicitSeedUrls.map((sourceUrl) => ({ sourceUrl }));
        console.log(`[Sync Liquipedia] Using ${dltvCatalogSeeds.length} explicit metadata seed URLs`);
      } else {
        try {
    ...
    let eventMetadataByUrl = new Map();
    if (syncCatalogMetadata || includeUpcomingEventMetadata) {
      const metadataSeedUrls = [
        ...dltvCatalogSeeds.map((row) => row.sourceUrl),
        ...(includeUpcomingEventMetadata ? liquipediaUpcoming.map((row) => row.eventUrl) : []),
      ].filter(Boolean);
      eventMetadataByUrl = await collectRelatedEventMetadata(metadataSeedUrls);
    }
    ...
    if (syncUpcoming) {
      ...
      const allTeams = await loadRelevantTeams(db, liquipediaUpcoming);
      ...
      for (const m of liquipediaUpcoming) {
        ...
        // Use the stable DLTV series ID directly as the record PK so that
        // ON CONFLICT updates start_time in-place when a match is rescheduled,
        // instead of leaving the old row and inserting a new duplicate.
        // For fallback entries (no DLTV series ID) use day-level granularity so
        // a reschedule within the same UTC day still deduplicates correctly.
        const id = m.seriesId
          ? seriesId
          : `${seriesId}_${Math.floor(m.timestamp / 86400)}`;

        // Remove any stale entries for this series that use the old timestamp-
        // suffixed id format (e.g. dltv_426173_1776499200).  This handles the
        // transition from the old scheme and any duplicates that slipped through
        // the bulk dedup step above.
        if (m.seriesId) {
          await db`DELETE FROM upcoming_series WHERE series_id = ${seriesId} AND id != ${id}`;
        }
    ...
    return {
      ...
      stats: {
        ...
        saved,
        catalogSeeds: dltvCatalogSeeds.length,
        dltvLogoBackfill,
      }
    };
  } catch (e) {
...
export async function upsertLiquipediaTournamentMetadata(input = {}) {
  const db = getDb();
  if (!db) {
    throw new Error('DATABASE_URL not configured');
  }
  ...
  const parsedUrlMeta = sourceUrl ? parseDltvEventUrl(sourceUrl) : null;
  ...
  const eventMeta = {
    ...
    title: normalizeString(input?.title) || parsedUrlMeta?.title || null,
    tier: normalizeString(input?.tier),
    location: normalizeString(input?.location),
    startTime: parseNumber(input?.startTime),
    endTime: parseNumber(input?.endTime),
    prizePool: normalizeString(input?.prizePool),
    prizePoolUsd: parseNumber(input?.prizePoolUsd),
    image: normalizeString(input?.image),
    locationFlagUrl: normalizeString(input?.locationFlagUrl),
    eventSlug: normalizeString(input?.eventSlug) || parsedUrlMeta?.eventSlug || null,
    parentSlug: normalizeString(input?.parentSlug) || parsedUrlMeta?.parentSlug || null,
    eventGroupSlug: normalizeString(input?.eventGroupSlug) || parsedUrlMeta?.eventGroupSlug || null,
  };
  ...
  const tournamentCandidates = await loadTournamentCandidates(db);
  await repairKnownTournamentRows(db, tournamentCandidates);
  const tournamentRecord = await resolveTournamentRecord(db, tournamentCandidates, eventMeta.title, eventMeta);

  return {
    success: true,
    leagueId: tournamentRecord.leagueId,
