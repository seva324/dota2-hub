...
function isFrontMatterLine(line = '', translatedTitle = '') {
  const value = String(line || '').trim();
  if (!value) return false;
  const plainTitle = String(translatedTitle || '').trim();
  if (plainTitle && value === plainTitle) return true;
  if (/^#{1,6}\s+/.test(value)) return true;
  if (/^(yesterday|today)$/i.test(value)) return true;
  if (/^\d+$/.test(value)) return true;
  if (/^\d+\s*(min\.?|mins?|minutes?|分钟|分|分钟阅读)$/i.test(value)) return true;
  if (/^\d{4}[年\-/.]\d{1,2}([月\-/.]\d{1,2}日?)?$/i.test(value)) return true;
  if (/^[*\-]\s{0,3}[A-Za-z][A-Za-z0-9_.-]{1,40}$/.test(value)) return true;
  return false;
}
...
export function sanitizeTranslatedArticleMarkdown(text = '', translatedTitle = '') {
  const normalized = normalizeParagraphs(stripMarkdownEmphasis(text));
  if (!normalized) return '';
  ...
  while (index < lines.length) {
    ...
    if (isFrontMatterLine(current, translatedTitle)) {
      index += 1;
      continue;
    }
    break;
  ...
  return normalizeParagraphs(lines.slice(index).join('\n'));
}
