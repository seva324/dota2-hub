...
function makeUnicodeRegex(term, { caseSensitive = false } = {}) {
  const normalized = normalizeAsciiTerm(term);
  if (!normalized) return null;
  const flexible = escapeRegExp(normalized)
    .replace(/\\ /g, '[\\s\\-]+')
    .replace(/\\'/g, "['’]?");
  return new RegExp(`(^|[^\\p{L}\\p{N}])(${flexible})(?=$|[^\\p{L}\\p{N}])`, caseSensitive ? 'u' : 'iu');
}

function makeUnicodeRegexGlobal(term, { caseSensitive = false } = {}) {
  const normalized = normalizeAsciiTerm(term);
  if (!normalized) return null;
  const flexible = escapeRegExp(normalized)
    .replace(/\\ /g, '[\\s\\-]+')
    .replace(/\\'/g, "['’]?");
  return new RegExp(`(^|[^\\p{L}\\p{N}])(${flexible})(?=$|[^\\p{L}\\p{N}])`, caseSensitive ? 'gu' : 'giu');
}

function makeAliasRegex(term, { caseSensitive = false } = {}) {
  return hasCyrillic(term)
    ? makeUnicodeRegex(term, { caseSensitive })
    : makeAsciiRegex(term, { caseSensitive });
}

function makeAliasRegexGlobal(term, { caseSensitive = false } = {}) {
  return hasCyrillic(term)
    ? makeUnicodeRegexGlobal(term, { caseSensitive })
    : makeAsciiRegexGlobal(term, { caseSensitive });
}
...
function buildMatcher(entry, category) {
  const safeTerms = [
    ...
    ...(Array.isArray(entry.russian_aliases) ? entry.russian_aliases : []),
  ]
  ...
  const matchers = safeTerms.map((term, index) => ({
    term,
    regex: makeAliasRegex(term, { caseSensitive: isUpperAlias(term) }),
...
function renderEntry(entry, category) {
  ...
  if (Array.isArray(entry.russian_aliases) && entry.russian_aliases.length) {
    aliasBits.push(`俄文别名：${entry.russian_aliases.join(' / ')}`);
  }
...
function replaceChineseAliases(text = '', entries = []) {
  ...
  for (const item of replacements) {
    output = output.replace(new RegExp(escapeRegExp(item.from), 'g'), (match, offset, whole) => {
      const start = Number(offset) || 0;
      const official = String(item.to || '');
      const officialLength = official.length;
      const aliasLength = match.length;

      for (let anchor = Math.max(0, start - officialLength + aliasLength); anchor <= start; anchor += 1) {
        if (whole.slice(anchor, anchor + officialLength) === official) {
          return match;
        }
      }
...
function replaceAsciiAliases(text = '', entries = []) {
  ...
  for (const item of replacements) {
    const regex = makeAliasRegexGlobal(item.from, { caseSensitive: item.caseSensitive });
    if (!regex) continue;
...
export function buildRequiredTranslationGlossaryPrompt(source = {}, options = {}) {
  const prompt = buildTranslationGlossaryPrompt(source, options);
  if (prompt) return prompt;
  return [
    '术语表（强约束）：本次标题或正文翻译必须执行术语统一。',
    '- 若原文出现英雄、物品、技能、地图机制、通用 Dota 术语，请优先使用官方/社区主流中文名。',
    '- 若没有把握，不要乱造新译名；保留专有名词原文。',
    '- 同一篇里术语前后一致，不要一个地方翻中文、另一个地方保留英文别名。',
  ].join('\n');
}
...
export function normalizeGlossaryTranslationsInMarkdown(text = '', source = {}) {
  const input = String(text || '');
  if (!input.trim()) return input;

  const placeholders = [];
  const protect = (value = '') => {
    const token = `__D2HUB_GLOSSARY_TOKEN_${placeholders.length}__`;
  ...
  let masked = input
    ...
    .replace(/\]\(([^)\s]+(?:\s+"[^"]*")?)\)/g, (_, destination) => `](${protect(destination)})`)
    .replace(/https?:\/\/[^\s)]+/g, (segment) => protect(segment));

  masked = normalizeGlossaryTranslations(masked, source);

  return masked.replace(/__D2HUB_GLOSSARY_TOKEN_(\d+)__/g, (_, index) => placeholders[Number(index)] ?? '');
}
