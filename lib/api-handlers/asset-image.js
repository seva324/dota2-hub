...
export default async function handler(req, res) {
  ...
  for (const candidate of candidates) {
    // Retry transient failures once — cold-start functions racing slow
    // upstream CDNs produced intermittent 502s on first paint.
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        const fetched = await fetchCandidate(candidate, req.method);
        if (!fetched.ok) {
          failures.push({ candidate, status: fetched.status, attempt });
          break;
        }
