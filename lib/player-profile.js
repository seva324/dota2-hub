...
export function summarizePlayerMatches(
  ...
  for (const row of sortedRows) {
    ...
    const player = players.find((p) => String(toInt(p?.account_id) ?? p?.account_id ?? '') === targetAccount);
    if (!player) continue;
  ...
  const signatureHero = [...heroEntries]
    .sort((a, b) => {
      if (b.win_rate !== a.win_rate) return b.win_rate - a.win_rate;
      if (b.matches !== a.matches) return b.matches - a.matches;
      return a.hero_id - b.hero_id;
  ...
  const wins = boundedRecent.filter((match) => match.won === true).length;
  const losses = boundedRecent.filter((match) => match.won === false).length;
  ...
  return {
    ...
    mostPlayedHeroes: heroEntries.slice(0, 5),
    signatureHero,
    wins,
    losses,
    decidedMatches,
