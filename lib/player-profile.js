...
export function normalizeLogo(url, req) {
...
function hasTeamIdentity(team) {
  ...
  return Boolean(teamId || teamName);
}

function isOfficialRecentMatch(match) {
  if (!match || typeof match !== 'object') return false;
  return Boolean(
    hasTeamIdentity(match.selected_team)
      && hasTeamIdentity(match.opponent)
  );
}
...
export function summarizePlayerMatches(
  ...
  const officialRecentMatches = recentMatches.filter((match) => isOfficialRecentMatch(match));
  const boundedRecent = officialRecentMatches.slice(0, recentLimit);
  const windowMatches = officialRecentMatches.filter((match) => (toInt(match?.start_time) || 0) >= windowStart);
